...
1. Codex 项目内 Research Operations 使用公开网站发现国内融资与广义资本原始线索；海外搜索当前暂停。
...
flowchart TB
    subgraph Discovery["研究发现层"]
        Research["Codex Research Operations<br/>国内媒体网站资本线索"]
        RSS["wechat-mp-rss<br/>待验证补充"]
        OpenAI["OpenAI<br/>抽取、核验、摘要"]
        Manual["人工补充链接"]
    end
...
### 3.1 Codex Research Operations

负责当前搜索 MVP 的国内融资与广义资本原始线索发现：

- 按 `domestic-website-search-list.md` 逐站执行公开网站宽搜。
- 使用融资、获投、并购、收购、IPO 等大事件词，不展开公司×关键词全量矩阵。
- 记录渠道、翻页范围、停止原因、访问限制和异常状态。
- 先形成原始线索池，经用户初筛后才转换为正式研究候选。

Research Operations 不能直接修改正式事件、内部判断或发布状态。WorkBuddy 已暂停；既有 C01/C02 WorkBuddy 文件与 CLI 仅作为兼容入口保留，不是现行发现主链路。

不直接爬取微信公众号。wechat-mp-rss 当前尚未验证，只能在实际产生可访问条目后补充原始线索，不能成为当前覆盖完成条件。
...
OpenAI输出必须通过固定 Schema 校验，只能进入候选层。海外网站发现当前暂停；未来恢复时由 Codex Research Operations 执行，OpenAI 不再承担地域搜索主责。既有 `openai-discover` 是暂停的兼容能力。
...
浏览器不直接调用飞书或 OpenAI。所有路由和公开数据都在构建时生成；链接、导航、前后周切换、筛选、排序、展开收起和图表切换可以使用浏览器端 JavaScript 交互。静态网站不等于不可点击，只是不承担登录、审核、实时写入或服务端数据库查询。
...
sequenceDiagram
    ...
    participant R as Codex Research Operations
...
- 07:00 后：处理已进入候选层的数据；搜索定时化完成实施和验收后再加入。
...
```mermaid
flowchart LR
    External["Research Operations / 兼容入口 / OpenAI / 外部网页"] --> Candidate["不可信线索或候选"]
...
- `lib/providers/`：OpenAI、兼容候选导入和通知 Adapter。
- `lib/feishu/`：飞书客户端、字段映射和 Repository。
- `lib/pipeline/`：候选处理、周报和调度；现有日报模块待迁移。
...
| 日报 | 当前真实表名与既有代码名称；目标语义迁移为每个北京时间自然周最多一条周报。迁移完成前不得声称周报已实现。 |
| 观察清单 | 保留历史关注对象与后续研究参考；当前网站搜索执行入口是仓库媒体清单，不由该表驱动。 |
| 内部战投备注 | 保存内部关注、判断、负责人和跟进状态；禁止进入公开投影。 |
| 自动化任务 | 保存定时任务的幂等、锁、重试、错误摘要和人工处理状态。 |

目标周报不存在独立来源板块。每个可发布融资事件必须关联至少一个可访问的原始来源，公开投影将链接内联到对应条目。人工确认的最终顺序优先。
...
### 9.18 C01 WorkBuddy 候选文件契约（遗留兼容）

C01 将既有 WorkBuddy 文件与项目内部处理流程通过版本化 JSON 文件解耦。WorkBuddy 已暂停；本节只保留已实现的兼容契约，不再定义现行搜索职责。候选文件仍不能携带正式事件、审核结果、发布控制或内部战投字段。
...
7. WorkBuddy 当前是暂停的兼容导入入口，不进入无人值守生产 SLA。
...
### 9.20 C03 OpenAI 研究 Provider 边界（海外发现入口暂停）

C03 把 OpenAI 分成供应商 Transport 和项目 Provider 两层。现有 Web Search 能力作为已实现兼容能力保留，但不属于当前搜索主链路；Provider 继续服务候选后的抽取、核验和摘要。领域层不依赖 OpenAI SDK。
...
### 9.30 D03 每日日报草稿生成（既有实现，待迁移）

D03 是已经完成的单日草稿实现，但产品目标已改为周报，当前不能作为最终周报能力验收。后续迁移任务必须按 `Asia/Shanghai` 汇总周一至周日已批准的唯一资本事件，并在次周一生成周报；生成只组织正式数据，不读取未批准候选，也不决定内容是否允许公开。
...
### 9.32 D05.1 指定日期海外候选发现入口（已实现、当前暂停）

D05.1 是已实现的 OpenAI 海外发现兼容入口，当前 MVP 不运行。若未来重新启用，必须另行确认其与 Codex Research Operations 海外搜索职责的衔接，且仍不得跨越人工审核边界。
...
1. 飞书多维表格始终是唯一正式数据源。
2. Research Operations、WorkBuddy 兼容入口和 OpenAI 只能产生原始线索、候选或建议。
3. 内部数据必须在公开投影前被隔离。
4. 网站必须能够从飞书正式数据重新生成。
