...
当前目标是于 **2026-08-07** 交付可在每个中国大陆法定工作日真实使用的MVP，跑通两个闭环：

1. 工作日情报：持续采集 → 去重 → Event聚合 → 评分/核验 → 审核 → 中国大陆法定工作日北京时间09:00发布 → Web/PDF/飞书；
...
- 响应式Web网站；
- 每日PDF；
- 飞书Agent，仅服务Admin私聊，支持09:00主动推送和只读问答；首选Hermes，生产接入不稳定时允许直接飞书应用/机器人降级。
...
- 来源可信度等级为A（90–100，官方一手/论文原文/官方代码）、B（75–89，主流或专业媒体/知名研究机构）、C（55–74，垂直小媒体/行业自媒体/已验证KOL或员工账号）、D（0–54，匿名/聚合转载/来源链或历史准确性未知）；
- A级通过校验可单独确认，两个不同证据链的B级可多来源确认，两个C级不能自动确认，D级只能待核实；等级调整必须版本化；
- Assessment分项使用0–100；满足证据门槛、相关性≥70、影响力≥60且无冲突或风险标记时，才可自动进入Brief；
- 相关性、事件影响、来源可信度、新颖性、Watchlist相关性、多来源验证的初始权重为25%、25%、20%、10%、10%、10%；综合分只用于排序和栏目；
- 已确认但影响力不足的内容进入“更多值得关注”；未达阈值的相关候选不得删除；
- 待核实、冲突、单一KOL或实体不明必须审核；IPO、并购、重大融资和重要模型开源进入优先审核；
...
- 从`2025-01-01`开始回溯；
- 19家全栈检查官方渠道及至少两个外部融资/新闻来源；5家大脑和7家本体检查官方渠道及至少一个外部来源；
- 每家公司保存检查来源、最后检查时间、覆盖状态和缺口；“未发现融资”必须有覆盖证据，空白不得解释为无融资；
- 高金额、轮次冲突、模糊金额和疑似重复事件必须人工抽查；
- 永久保留原始金额、币种、原文、汇率来源/日期/方法及人民币换算值；
- 汇率优先使用正式公告日；当天无值时使用此前最近可用工作日；只知月份时使用月末最后可用工作日并标记估算；
- 后续补充披露不得覆盖旧换算，只有交易日期被正式纠正时才创建新版本并重算；
- 未披露或`unknown`不等于0；
- 模糊金额可以形成明确标注的区间，不得伪造精确值；
- 首页“已披露融资总额”只含股权融资与战略投资；
- 债务/授信、补贴、IPO再融资、并购交易额分别统计；
- 核验、人工审核、发布、是否进入统计必须是独立状态；
- 独立来源必须具有不同原始证据链；转载同一报道、引用同一爆料或复述同一社交帖子只算一个来源；
- 官方一手来源仍须通过实体匹配、关键字段校验和冲突检查；单一KOL或信息链不明内容只能进入待核实；
...
- 仅中国大陆官方工作日历标记的工作日运行早间调度，调休补班日运行，非工作日跳过；
- 08:00：融资审核快照；
- 08:30：Brief候选截止并生成快照；
- 09:00：Web Brief发布和飞书递送；
- PDF可异步完成，失败不得阻塞Web。

三个早间任务必须独立、幂等、可审计，不得写成一个巨大Cron。非工作日仍持续采集、处理、核验和更新满足门槛的融资看板；不生成Brief、PDF或飞书递送。待审核事件保留到下一工作日，其他期间事件进入下一工作日Brief。

长假后核心Brief仍限制为15–25条并保留3–5条重点；超出部分进入“假期更多动态”，显示累计候选数、核心收录数和覆盖区间。不得为了篇幅删除相关Event，PDF只含核心并链接Web完整列表。
...
云端采集器处理不需要个人登录的公开来源。Codex与WorkBuddy都负责登录态来源：WorkBuddy优先覆盖微信、小红书、B站等中国App，Codex同时负责X、LinkedIn及其他需要登录的平台；双方均可补充对方覆盖。
...
Codex与WorkBuddy可以自动提交候选，无需Admin逐条预批准。提交成功只能标记为“已接收/待处理”，不得直接标记为已核验、已入库或已发布。

凭证规则：

- 个人Cookie、Session、Token和Browser Profile只保留在本地；
- Codex与WorkBuddy使用不同、可撤销且最小权限的设备Credential；
...
Hermes是MVP在Admin飞书私聊中的首选对话层。若Hermes生产接入不稳定，可以使用直接飞书应用/机器人作为降级对话层；降级不能削弱只读权限、来源引用、限流或审计。
...
Hermes与直接飞书降级Adapter在MVP中都严格只读：
...
- 一个有效Scheduler负责一套计划；
- 失败任务可见、可审计、可重放；
- 重放不得产生重复Event、FinancingEvent、Brief条目、递送或累计金额；
...
负责Source Registry、Adapter、原始抓取、Checkpoint、Health，以及Codex/WorkBuddy本地采集输入规范化；不负责最终Event合并、评分、发布或融资统计。
...
MVP达到`design-document.md`第13节时可于2026-08-07上线。7个工作日周期必须通过可控时钟集成测试；真实09:00连续工作日运行从首个可用工作日累计，8月7日不足7个工作日时报告实际数量并上线后补足，不得伪造，也不因日历窗口不足单独阻塞MVP。五项质量指标在上线后的连续4周验证，不阻塞8月7日上线。
