...
type AmountSummary = {
  ...
  singleRoundRanking: Array<{rank: number; company: string; originalAmount: string; normalizedAmount: string; basis: string; note: string}>;
};
...
export function DashboardClient({rows, amountSummary, basePath}: {rows: DashboardRow[]; amountSummary: AmountSummary; basePath: string}) {
  ...
  return <main className="dashboard-page">
    ...
    <header className="dashboard-header"><p className="eyebrow">INTERACTIVE FUNDING DATA</p><h1>融资数据面板</h1><p className="deck">浏览{weeks.length}期公开周报数据；筛选与搜索均在浏览器本地完成，不包含飞书候选或内部备注。</p></header>
    <section className="dashboard-filters" aria-label="数据筛选">
      <label>周次<select value={week} onChange={(event) => setWeek(event.target.value)}><option value="ALL">{allIssuesLabel}</option>{weeks.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
